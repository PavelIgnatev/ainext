...
function validateCharacters(message: string) {
  ...
  const hasLinks =
    /(?:[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})|(?:(?:https?:\/\/)?(?:(?:\d{1,3}\.){3}\d{1,3}|(?:[A-Za-z0-9](?:[A-Za-z0-9-]*[A-Za-z0-9])?\.)+[A-Za-z0-9-]+)(?::\d+)?(?:\/\S*)?)/gi.test(
      message
    );
  if (hasLinks) {
    return {
      isValid: false,
      error: 'Links are not allowed in ping messages',
    };
  }
  return { isValid: true };
}
...
export function llmPingValidation(message: string, stage?: number) {
